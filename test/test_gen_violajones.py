...
def test_all_points_in_same_block():
	...
	for s, gt in test_shapes:
		assert gen_code.all_points_in_same_block(gen_points(s), block_size) == gt
...
def gen_faces_detect_no_errors():
	...
	code = Code()
	def code_gen(code): return gen_code.gen_detect_faces(code, cascade, block_size)
	code.set_generator(code_gen, block_size)
...
@skip_test # skip this test until problem is fixed with the calculation of the read points
def test_detect_faces():
	...
	def run_test(image, cascade):
		...
		code = Code()
		code.set_generator(gen_code.gen_detect_faces, block_size, args)
...
def test_detect_faces_fullintegral():
	...
	def run_test(image, cascade):
		...
		# now execute the codegen
		code = Code()
		code.set_generator(gen_code.gen_detect_faces_fullintegral, block_size, args)
		#print '# instructions: %i'%(code.instr_size())
...
def test_no_manual_alloc():
	...
	cascade_filename = '../data/haarcascade_frontalface_alt.xml'
	cascade = parse_haar.parse_haar_xml(cascade_filename)
	cascade.stages = cascade.stages[0:5] # reduce nr of stages to improve test time
	...
	print cnt
	code2 = TestCode()
	code2.set_generator(gen_code.gen_detect_faces_fullintegral, block_size, args)
